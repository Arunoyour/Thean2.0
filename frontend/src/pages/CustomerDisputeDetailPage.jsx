  ...
  return (
    <div className={`dispute-bubble-wrap ${isAdmin ? "admin" : "user"}`}>
      <div className="dispute-bubble">
        ...
        {msg.voice_file_path && (
          <audio controls src={`${API_BASE}/../${msg.voice_file_path}`} className="dispute-audio" />
        )}
        {msg.image_file_path && (
          <img src={`${API_BASE}/../${msg.image_file_path}`} alt="attachment" className="dispute-img" />
        )}
        {msg.attachment_path && (
          <a href={`${API_BASE}/../${msg.attachment_path}`} target="_blank" rel="noreferrer" className="dispute-file-link">
            📎 {msg.attachment_name || "Attachment"}
          </a>
        )}
        <time className="dispute-bubble-time">{fmtDateTime(msg.created_at)}</time>
      </div>
    </div>
  );
}
...
export function CustomerDisputeDetailPage() {
  ...
  async function load() {
    setIsLoading(true);
    try { setDispute(await getOrderDispute(disputeId)); }
  ...
  async function sendReply(e) {
    ...
    if (audioBlob) {
      fd.append("voice_file", new File([audioBlob], "reply.webm", { type: "audio/webm" }));
      fd.append("voice_duration_secs", String(recordingSec));
    }
    if (attachment) fd.append(attachment.type.startsWith("image/") ? "image_file" : "attachment_file", attachment);

    setIsSending(true);
    try {
      await replyOrderDispute(disputeId, fd);
      setReplyText(""); setAudioBlob(null); setAttachment(null);
      await load();
  ...
  async function handleClose() {
    if (!window.confirm("Close this dispute? You can raise a new one on the same order later.")) return;
    setIsClosing(true);
    try { await closeOrderDispute(disputeId); await load(); }
    catch (err) { setError(err.message); }
    finally { setIsClosing(false); }
  }

  if (isLoading) return <div className="ph-page ph-loading"><p>Loading…</p></div>;
  if (!dispute) return <div className="ph-page"><p>Dispute not found.</p></div>;
    ...
    <div className="ph-page dispute-detail-page">
      <header className="ph-inner-header">
        ...
        <div className="dispute-detail-title">
          <h1>Dispute</h1>
          <span className="dispute-status-pill" style={{ background: statusColor }}>{STATUS_LABEL[dispute.status]}</span>
        </div>
      </header>
      ...
      <div className="dispute-meta">
        ...
        {(dispute.tagged_sectors || []).map((s) => (
          <span key={s} className="dispute-sector-tag">{s}</span>
        ))}
        <span>{dispute.age_days} days old</span>
      </div>
        ...
        <>
          <form className="dispute-reply-form" onSubmit={sendReply}>
            ...
            <div className="dispute-reply-actions">
              {audioBlob ? (
                <div className="dispute-audio-mini">
                  <audio controls src={URL.createObjectURL(audioBlob)} />
                  <button type="button" className="dispute-remove-btn" onClick={() => setAudioBlob(null)}><X size={12} /></button>
                </div>
              ) : isRecording ? (
              ...
              {attachment ? (
                ...
                <label className="dispute-attach-btn">
                  <Paperclip size={16} />
                  <input type="file" accept="image/*,application/pdf,audio/*" style={{ display: "none" }} onChange={pickFile} />
                </label>
              )}
              <button type="submit" className="dispute-send-btn" disabled={isSending}>
                <Send size={16} />
              </button>
            </div>
          </form>

          <button type="button" className="dispute-close-btn" disabled={isClosing} onClick={handleClose}>
      ...
      {isClosed && (
        <div className="dispute-closed-banner">
          ...
          <button type="button" className="dispute-raise-again-btn"
            onClick={() => navigate(`/home/disputes/raise?order_id=${dispute.source_order_id}&sectors=${(dispute.tagged_sectors||[]).join(",")}`)}>
