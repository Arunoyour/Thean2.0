...
export function CustomerRaiseDisputePage() {
  ...
  const defaultSectors = params.get("sectors") || "pharmacy";

  const [sectors, setSectors] = useState(
    defaultSectors.split(",").map((s) => s.trim()).filter(Boolean)
  );
  ...
  async function submit(e) {
    e.preventDefault();
    setError("");
    if (!orderId) { setError("Order ID missing — go back and try again."); return; }
    if (!text.trim() && !audioBlob && !attachment) { setError("Add text, a voice note, or a file."); return; }
    ...
    if (audioBlob) {
      fd.append("voice_file", new File([audioBlob], "voice.webm", { type: "audio/webm" }));
      fd.append("voice_duration_secs", String(recordingSec));
    }
    if (attachment) {
      fd.append(attachment.type.startsWith("image/") ? "image_file" : "attachment_file", attachment);
    }

    setIsSubmitting(true);
    try {
      await raiseOrderDispute(fd);
  ...
  return (
    <div className="ph-page">
      <header className="ph-inner-header">
        <button type="button" className="ph-back-btn" onClick={() => navigate(-1)}>
          <ArrowLeft size={20} />
        </button>
        <h1>Raise Dispute</h1>
      </header>
      ...
      <form className="dispute-raise-form" onSubmit={submit}>
        ...
        <div className="dispute-field">
          <label className="dispute-label">Order reference</label>
          ...
          <div className="dispute-sector-chips">
            {["pharmacy", "delivery"].map((s) => (
              <button
                key={s}
                type="button"
                className={`dispute-sector-chip${sectors.includes(s) ? " active" : ""}`}
        ...
        <div className="dispute-field">
          ...
          {audioBlob ? (
            <div className="dispute-audio-preview">
              <audio controls src={URL.createObjectURL(audioBlob)} />
              <button type="button" className="dispute-remove-btn" onClick={() => setAudioBlob(null)}>
                <X size={14} /> Remove
              </button>
            </div>
          ) : isRecording ? (
            <button type="button" className="dispute-record-btn recording" onClick={stopRecording}>
              <MicOff size={18} /> Stop ({recordingSec}s)
            </button>
          ) : (
            <button type="button" className="dispute-record-btn" onClick={startRecording}>
        ...
        <div className="dispute-field">
          ...
          {attachment ? (
            <div className="dispute-attachment-preview">
              ...
              <button type="button" className="dispute-remove-btn" onClick={() => setAttachment(null)}>
                <X size={14} />
              </button>
            </div>
          ) : (
            <label className="dispute-file-label">
        ...
        <button type="submit" className="dispute-submit-btn" disabled={isSubmitting}>
          <Send size={16} /> {isSubmitting ? "Submitting…" : "Submit Dispute"}
        </button>
      </form>
    </div>
  );
