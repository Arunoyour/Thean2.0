...
export function DeliveryDisputeDetailPage() {
  ...
  async function load(){setIsLoading(true);try{setDispute(await getDeliveryOrderDispute(disputeId));}catch(e){setError(e.message);}finally{setIsLoading(false);}}
  ...
  async function sendReply(e){
    ...
    setIsSending(true);
    try{await replyDeliveryOrderDispute(disputeId,fd);setReplyText("");setAudioBlob(null);setAttachment(null);await load();}
  ...
  async function handleClose(){
    ...
    setIsClosing(true);
    try{await closeDeliveryOrderDispute(disputeId);await load();}catch(err){setError(err.message);}finally{setIsClosing(false);}
  }

  if(isLoading)return <div className="dl-page"><p className="dl-loading">Loading…</p></div>;
  if(!dispute)return <div className="dl-page"><p>Not found.</p></div>;

  const isClosed=dispute.status==="CLOSED";

  return(
    <div className="dl-page dispute-detail-page">
        ...
        <>
          <form className="dispute-reply-form" onSubmit={sendReply}>
            ...
            <div className="dispute-reply-actions">
              {audioBlob?(<div className="dispute-audio-mini"><audio controls src={URL.createObjectURL(audioBlob)}/><button type="button" className="dispute-remove-btn" onClick={()=>setAudioBlob(null)}><X size={12}/></button></div>)
              :isRecording?(<button type="button" className="dispute-mic-btn recording" onClick={stopRecording}><MicOff size={16}/> {recordingSec}s</button>)
              :(<button type="button" className="dispute-mic-btn" onClick={startRecording}><Mic size={16}/></button>)}
              {attachment?(<span className="dispute-attach-name"><Paperclip size={12}/> {attachment.name} <button type="button" onClick={()=>setAttachment(null)}><X size={12}/></button></span>)
